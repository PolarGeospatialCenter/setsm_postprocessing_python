...
fprintf(2, ['' ...
        ...
        '    :: Browse TESTDIR, may use "[]" as wildcard.\n' ...
        'list :: Show current BROWSE and SELECTION.\n' ...
        'add [browse_1] [browse_2] ... [browse_n]\n' ...
        '    :: Add images to SELECTION by current BROWSE index.\n' ...
        'remove [selection_1] [selection_2] ... [selection_n]\n' ...
        '    :: Remove images from SELECTION by current SELECTION index.\n' ...
        'delete [selection_1] [selection_2] ... [selection_n]\n' ...
        '    :: Delete images from TESTDIR by current SELECTION index.\n' ...
        'refresh :: Update BROWSE and SELECTION with current TESTDIR contents.\n' ...
        'view {img/ras} {noimg} {nohist} {nodata=x} {big} [selection_1] [selection_2] ... [selection_n]\n' ...
        '    :: View images separately by current SELECTION index.\n' ...
        'compare {img/ras} {noimg} {nohist} {nodata=x} {masknans} {showcast} {split} {difflate} {big} [selection_1 selection_2]\n' ...
        '    :: Compare two images by current SELECTION index.\n' ...
        'auto {img/ras} {noimg} {nohist} {nodata=x} {masknans} {showcast} {split} {difflate} {big}\n' ...
        '    :: Compare ml/py image pairs as saved by SRA/SIA, view others.\n' ...
...
while ~ready
    ...
    try
        ...
        if any(strcmp(command_args(1), ["view", "comp", "compare", "auto"]))
            ...
            if isempty(selection)
                ...
                if length(command_args) > 1
                    ...
                    nodata_arg = find(cellfun(@(y) ~isempty(y), (arrayfun(@(x) find(strfind(x, ["nodata="])), check_options, 'UniformOutput', false))));
                    if ~isempty(nodata_arg)
                        nodata_str = strrep(check_options(nodata_arg), "nodata=", "");
                        nodata_val = str2double(nodata_str);
                        if isnan(nodata_val) && ~strcmp(lower(nodata_str), "nan")
                            errmsg = sprintf("Invalid nodata argument '%s'.", nodata_str);
                            error('CUSTOM MESSAGE');
                        end
                    end
                    ...
                    if ~isempty(cell2mat(arrayfun(@(x) find(strcmp(x, ["big"])), check_options, 'UniformOutput', false)))
                        display_small = false;
                    end
                end
                if image_type ~= -1
        ...
        if strcmp(ME.message, 'CUSTOM MESSAGE')
            ;
        elseif strcmp(ME.message, 'Index exceeds array bounds.') ...
            || strcmp(ME.message, 'Index exceeds matrix dimensions.') ...
