...
def saveStripBrowse(args, demFile, demSuffix, maskSuffix):
    ...
    if demFile_10m in output_files:
        demFile_float = demFile
        if args.get(ARGSTR_RES) == 0.5:
            ds = gdal.Open(demFile, gdal.GA_ReadOnly)
            if ds.GetRasterBand(1).DataType != gdalconst.GDT_Float32:
                output_files.add(demFile_unscale_temp)
                commands.append(
                    ('gdal_translate -q -unscale -ot Float32'
                     ' -co TILED=YES -co BIGTIFF=YES -co COMPRESS=LZW'
                     ' "{0}" "{1}"'.format(demFile, demFile_unscale_temp))
                )
                demFile_float = demFile_unscale_temp
        commands.append(
            ('gdalwarp -q -overwrite -tr {2} {2} {3} -r bilinear -dstnodata -9999'
             ' -co TILED=YES -co BIGTIFF=YES -co COMPRESS=LZW -co PREDICTOR=3'
             ' "{0}" "{1}"'.format(demFile_float, demFile_10m_temp, 10, tap_arg))
        )
        commands.append(
