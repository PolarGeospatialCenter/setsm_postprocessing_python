...
# Modify the two code blocks below to set per-script and default environment load commands

## Set environment load commands specific to batch scripts
#if [ "$job_class" == "s2s" ]; then
#    env_load_cmd="module load mod_name"
#fi

## Set default environment load command for all batch scripts (fallback if no specific
