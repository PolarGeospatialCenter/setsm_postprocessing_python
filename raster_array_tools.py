...
def test_setRunnum(new_runnum=None, increment=False, concurrent=False):
...
def test_incRunnum(concurrent=False):

    return test_setRunnum(increment=True, concurrent=concurrent)


def test_getNextImgnum(runnum=test_getRunnum(), compare=False, concurrent=False):
    next_imgnum = -1
...
def test_saveImage(array, PILmode, fname='testImage_py.tif'):
    ...
    image.save(testFile)
    # misc.imsave(testFile, array)
    print "'{}' saved".format(testFile)


def test_saveImageAuto(array, flavor, keyword='', descr='', compare=False, concurrent=False):
    ...
    test_saveImage(array, PILmode, fname=testFname)


def test_saveRaster(array, fname='testRaster_py.tif',
                    X=None, Y=None, proj_ref=None, geotrans_rot_tup=(0, 0),
                    like_rasterFile=None,
                    nodataVal=None, dtype_out=None, force_dtype=False, skip_casting=False):
    ...
    saveArrayAsTiff(array, testFile,
...
def test_saveRasterAuto(Z, X, Y, flavor, keyword='', descr='', compare=False, concurrent=False, proj_ref=None):
    ...
    test_saveRaster(Z, X, Y, proj_ref=proj_ref, nodataVal=nodata, dtype_out=fmt, fname=testFname)
    Z[np.where(Z == -9999)] = np.nan


def test_waitForComparison(expected_imgnum):

    last_imgnum = test_getNextImgnum(compare=True, concurrent=True)
...
def test_shape(demFile):
    ...
    Z, X, Y, proj_ref = oneBandImageToArrayZXY_projRef(demFile)
    poly = getDataBoundariesPoly(Z, X, Y, nodataVal=-9999)
    if not poly:
